...
class LS {
  ...
  public static $default_config = array(
     ...
     */
    "info" => array(
      "company" => "Scambidilibri",
      "email" => "scambidilibri@scambidilibri.it"
    ),
    ...
    "db" => array(
      ...
      "port" => 3306,
      "username" => "u431533919_admin",
      "password" => "galilei1600",
      "name" => "u431533919_data",
      "table" => "users"
    ),
    ...
    "pages" => array(
       ...
       */
      "no_login" => array(
        "/", "/index.php", "/register.php", "/confirm.php", "/forgotPassword.php"
      ),
      /**
  ...
  public static $dbh; // prima era private
  ...
  public static function init() {
	...
	Materialize.toast("'.$_GET['msg'].'", 4000);
	</script></body>';
	}
	print '</html>';
	//Fine parte aggiunta
  ...
  public static function logout(){
    ...
    session_destroy();
    setcookie("logSyslogin", "", time()-3600, self::$config['cookies']['path'], self::$config['cookies']['domain']); //c'era un bug
    setcookie("logSysrememberMe", "", time()-3600, self::$config['cookies']['path'], self::$config['cookies']['domain']); // c'era un bug
    self::redirect(self::$config['pages']['login_page']);
    return true;
  }
    ...
    if( !isset($_POST['logSysForgotPass']) && !isset($_GET['resetPassToken']) && !isset($_POST['logSysForgotPassChange']) ){
      ...
      if($identification == ""){
        ...
        if($sql->rowCount() == 0){
           ...
           */
          $subject = "Reset Password - Scambidilibri";
          $body   = "Per cambiare la password fai click sul seguente link:
          <blockquote>
            <a href='". self::curPageURL() ."?resetPassToken={$encodedToken}'>Reset Password : {$token}</a>
          </blockquote>";
          \Fr\LS::sendMail($email, $subject, $body);
